...
def bbox_overlaps(boxes, query_boxes):
    ...
    iw = (np.minimum(boxes[:, 2:3], query_boxes[:, 2:3].transpose()) - np.maximum(boxes[:, 0:1], query_boxes[:, 0:1].transpose())).clip(min=0)
    ih = (np.minimum(boxes[:, 3:4], query_boxes[:, 3:4].transpose()) - np.maximum(boxes[:, 1:2], query_boxes[:, 1:2].transpose())).clip(min=0)

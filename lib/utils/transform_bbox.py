...
def bbox_transform_inv(boxes, deltas):
    ...
    :param deltas: B * n * 4 numpy array, pred_boxes boxes
    :return: pred_boxes: B * n * 4 numpy array, pred_boxes boxes
    """
    if boxes.shape[0] == 0:
    ...
    # numpy.ndarray
    # (B, n, 1) x (n, 1) = (B, n, 1)
    pred_ctr_x = dx * widths[:, np.newaxis] + ctr_x[:, np.newaxis]  # B * n * 1
    pred_ctr_y = dy * heights[:, np.newaxis] + ctr_y[:, np.newaxis]
    pred_w = np.exp(dw) * widths[:, np.newaxis]
    pred_h = np.exp(dh) * heights[:, np.newaxis]

    pred_boxes = np.zeros(deltas.shape, dtype=deltas.dtype)  # B * n * 4
    # x1
...
def clip_boxes(boxes, im_shape):
    ...
    # x1 >= 0
    # np.minimum(a,b), every element in a must small than b
    # the same as np.maximum
